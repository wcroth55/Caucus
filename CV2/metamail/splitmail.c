...
/*  Modifications by Charles Roth, CaucusCare.  Published under
/   the same conditions as above. */

/*: CR 11/12/03 Get the location of sendmail from env var METAMAIL_SENDMAIL. */
...
#else
extern char *getenv();
#define NORMALDELIVERYCMD  " -t -oi"
#define VERBOSEDELIVERYCMD " -t -v -oi"
#endif
...
char **argv;
{
    int i, DoDeliver=0, SplitSize=DEFAULT_SPLIT_SIZE, dum, InNewline=1, 
        bytesread, whichpart=1, Verbose=0, numparts = -1, c;
    char *fname = NULL, *bigbuf, *s, *SharedHeaders, *headend, *from, id[100], 
         deliverycmd[512], *prefix, SubjectBuf[250];
    ...
    s = getenv ("METAMAIL_SENDMAIL");
    if (s) strcpy (sendmail, s);
    else   strcpy (sendmail, "/usr/sbin/sendmail");
	...
	*s = '\n';
    ...
    strcpy (deliverycmd, sendmail);
    strcat (deliverycmd, Verbose ? VERBOSEDELIVERYCMD : NORMALDELIVERYCMD);
    while (!feof(fp)) {
