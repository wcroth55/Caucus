...
/*** JIXOFSJIS.  Convert stream of 8-bit bytes in SJIS to Jix.
/
/    code = jixofsjis (a, b, init, outseq);
/
/    Parameters:
/       int   code;
/       Chix  a;
...
/       array of 8-bit bytes in B in the SJIS format, into jix, and
...
/
/    How it works:
/       Jixofsjis() converts any number of incoming SJIS bytes into Jix, and
/       adds them on to the end of A.  Jixofsjis can handle a variety of
/       special cases, including:
/
/         1. B contains a single byte of a multi-byte SJIS character
/            (Jixofsjis keeps an internal state machine to keep track of
...
/      -3   Error!  Jix A cannot be expanded larger.
/
...
/*: CR  6/18/91 19:55 Use 0xA1 as base of EUC/jix, not 0x81. */
/*: CR  1/03/92 13:41 Return -3 if A cannot be expanded. */
/*: CX  6/04/92 14:00 Add "intermediate state" return code. */
/*: CR 12/11/92 13:23 Chxresize last arg is int4 *. */

#include <stdio.h>
#include "chixdef.h"

FUNCTION  jixofsjis (a, b, init, outseq)
   Chix   a;
   char  *b;
   int    init;
   char  *outseq;
{
   ...
   static int4 pos;
   static int  state;
   int    i, code, highbyte, byte, offset;
   ...
   if (NOT chxcheck (a, "jixofsjis"))  RETURN (-1);
...
/*    if (byte != 0x0A  &&  byte < 040)  { code = 2;   continue; } */
      ...
      if (state == 0) {
         if (byte == 0x80  ||  byte >= 0xF0)  { code = 2;   continue; }

         if      (byte >= 0xA0  &&  byte <= 0xDF) {  /* sw-kana. */
            a->p [i  ] = 0x8E;
            a->p [i+1] = byte;
            ++pos;
         }

         else if (byte >= 0x81  &&  byte <= 0x9F) {     /* 1st bank of jis. */
            a->p [i  ] = (byte - 0x80 - 1) * 2 + 0xA1;  /* seg #s 1, 3, 5... */
            a->p [i+1] = '\0';
            state = code = 1;
         }

         else if (byte >= 0xE0  &&  byte <= 0xEF) {  /* 2nd bank of jis. */
            a->p [i  ] = (byte - 0xE0 + 32 - 1) * 2 + 0xA1;
            a->p [i+1] = '\0';
            state = code = 1;
         }
